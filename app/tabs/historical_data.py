...
from helpers.data_views import (
    list_team_choices,
    list_seasons_for_team,
    team_season_view,
    rows_for_compare,
)
...
CART_KEY = "compare_cart"  # session key for the compare cart
...
def render():
    ...
    # ---------------- Selectors (cached lists) ----------------
    with timed("load team list"):
        teams = list_team_choices(processed_path, COLLEGE_MAP)

    team_display = st.selectbox("Team", teams, index=0 if teams else None)
    selected_norm = (
        COLLEGE_MAP_INV.get(team_display, str(team_display).strip().lower()) if team_display else None
    )

    with timed("load seasons for team"):
    ...
    # Toggle edit mode to avoid heavy editor on every rerun
    ...
    # Download (no compare col)
    download_display = view_display.drop(columns=[c for c in ["Compare?"] if c in view_display.columns], errors="ignore")
    st.download_button(
    ...
    # ---------------- Possible lineup (stable + cached geometry) ----------------
    st.markdown("---")
    st.subheader("Possible lineup")

    # Choose minutes col fallback
    minutes_col = None
    ...
    if not minutes_col:
        ...
        with timed("build lineup payload"):
            ...
            for r in rows:
                ...
                try:
                    ...
                    if efg is not None and efg <= 1.0:
                        efg *= 100.0
                except Exception:
                    efg = None
        ...
        # unique keys prevent the figure from disappearing when other Plotly comps render
        lineup_plotly_key = f"lineup_click_{selected_norm}_{season}"
        try:
            selected = plotly_events(
                fig, click_event=True, hover_event=False, select_event=False, key=lineup_plotly_key
            )
            if selected:
    ...
    # ---------------- Comparison (wrapped in expander, cached data reads) ----------------
    st.markdown("---")
    with st.expander("Comparison", expanded=False):
        if CART_KEY not in st.session_state:
            st.session_state[CART_KEY] = []

        # add checked (only when editor is on)
        if edit_mode and st.button("Add checked to compare"):
            ...
            if checked.empty:
                ...
                # after rename_columns, player_ind is "Player Name"
        ...
        if cart:
            ...
            # Grab matching rows using cached, pruned read
            ...
            if rows:
                ...
                # Side-by-side table (pretty headers)
                show_cols = [
